  ...
  return (
    <div className="p-10 min-h-screen bg-gradient-to-br from-blue-100 via-blue-50 to-white font-sans">
      <div className="max-w-7xl mx-auto">
        {/* Owner username on top */}
        {ownerUser && (
          <div className="mb-2 flex items-center gap-2">
            <span className="font-semibold text-green-700">Owner:</span>
            <span className="bg-green-100 text-green-800 px-3 py-1 rounded-full text-sm font-medium">{ownerUser.username}</span>
          </div>
        )}
        {/* Collaborator usernames on top */}
        {collaboratorUsers.length > 0 && (
          <div className="mb-4 flex flex-wrap gap-3 items-center">
            <span className="font-semibold text-blue-700">Collaborators:</span>
            {collaboratorUsers.map(u => (
              <span key={u.id} className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm font-medium">
                {u.username}
              </span>
            ))}
          </div>
